...
Every tweet, every geo-tagged picture, every phone call, and every purchase with credit card, provide the user's location not only to the primary service, but also to all the applications and services that have been authorized to access these data.
This flow of information was a crucial part of the Web 2.0 revolution, realized with RESTful APIs, mashups, and authorizations.
The complexity of the flow has now became too large for a user to handle and manage.

Increasing the amount of data the user controls and granularity of the control is meaningless if this control cannot be exercised in an informed way.
For many years the End User License Agreements (EULAs), long incomprehensible texts, have been often accepted blindly by the end user, trusting that they are not agreeing to anything that could harm them.
